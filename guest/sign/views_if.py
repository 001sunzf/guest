...
def add_guest(request):
    ...
    try:
        Guest.objects.create(realname=realname,phone=int(phone),email=email,sign=0,enevt_id=int(eid))
...
def user_sign(request):
    ...
    result = Event.objects.get(id=eid).status
    if not result:
        return JsonResponse({'status':10023,'message':'evnet staus is not available!'})
    ...
    if result:
        ...
        Guest.objects.filter(event_id=eid,phoe=phone).update(sign='1')
